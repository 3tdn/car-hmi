...
> Implementation update (2026-09-26): signal sample history was removed. Any section below
> mentioning `signal_log`, history queries, retention, batch persistence, or signal export is a
> historical design and is superseded by [Signal History Removal Report](signal_history_removal.md).
> The realtime path ends at the in-memory `SignalStore`; SQLite keeps only `signal_config`.
>
