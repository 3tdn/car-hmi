...
| `PATCH` | `/config/signal/{name}` | Cập nhật cấu hình tín hiệu (cần `full`) |
| `GET` | `/config/processor` | Cấu hình processor hiện tại |
| `POST` | `/config/processor` | Cập nhật processor config (cần `full`, áp dụng live) |
| `GET` | `/config/general` | Toàn bộ app config (JSON) |
| `PATCH` | `/config/general` | Cập nhật một phần app config (cần `full`) |
| `POST` | `/config/general/reset` | Reset app config về mặc định (cần `full`) |
| `GET` | `/config/alarms` | Cấu hình ngưỡng cảnh báo (raw YAML → JSON) |
| `POST` | `/config/alarms` | Cập nhật toàn bộ alarms config (cần `full`) |
| `POST` | `/config/alarms/reset` | Reset alarms config về rỗng (cần `full`) |
